...
flowchart LR
  ...
  F --> L["JsonCodecRenderer"]
  F --> M["WktJsonCodecRenderer"]
...
| `WktJsonCodecRenderer` | 只替换 WKT 的 canonical JSON 表示，不改 wire codec   |
| `runtime/wire/`        | 在 ArkTS 侧实现 wire reader、writer 与 wire type     |
| `runtime/json/wkt/`    | WKT JSON 格式、解析和 Any 类型注册表                 |
| `ProtoContainers`      | 生成 codec 使用的 Sendable Array/Map 容器适配 facade |
