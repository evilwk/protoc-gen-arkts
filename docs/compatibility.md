...
生成器的 `0.4.x` 把现有 proto3、nested、repeated、packed、map、oneof、跨文件、跨组 import 和 enum 行为视为稳定契约。
...
runtime HAR 采用独立的 `1.0.x` 包版本，以符合当前 HarmonyOS 包工具对版本号的校验规则；它与生成器版本不要求数值相同。正式发布后以如下兼容矩阵为准：
